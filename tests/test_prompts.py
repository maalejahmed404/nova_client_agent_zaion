...
def test_a_situation_counts_only_if_every_element_is_established():
    verdict = prompts.PostReviewVerdict(documents_cover=True, needs=[], advisor_conditions=[], reason="", candidates=[
...
def test_an_advisor_is_needed_only_if_no_documented_condition_fails():
    def verdict(*met):
        return prompts.PostReviewVerdict(candidates=[], needs=[], documents_cover=True, reason="",
    ...
    assert prompts.PostReviewVerdict(candidates=[], needs=[], documents_cover=False, advisor_conditions=[], reason="").can_conclude
    matched = [{"item": 2, "elements": [{"element": "e", "established": True}]}]
    assert not prompts.PostReviewVerdict(candidates=matched, needs=[], documents_cover=False, advisor_conditions=[], reason="").can_conclude

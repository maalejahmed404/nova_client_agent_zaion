...
def report(rows: list[dict]) -> None:
    ...
    archive_leak_count = sum(
        len(r["archive_leak"]) > 0 for r in rows if not r.get("historical", False)
    )
    ...
    if with_answer:
        ...
        print(
            f"answer in corpus         min {with_similarities[0]:.3f}  median {statistics.median(with_similarities):.3f}  max {with_similarities[-1]:.3f}"
        )

    if without_answer:
        without_similarities = sorted(r["similarity"] for r in without_answer)
        print(
            f"answer not in corpus     min {without_similarities[0]:.3f}  median {statistics.median(without_similarities):.3f}  max {without_similarities[-1]:.3f}"
        )
    ...
    if with_answer and without_answer:
        ...
        print(
            f"a threshold above every negative ({max_negative:.3f}) would also cut {len(wrongly_cut)} answerable questions: {wrongly_cut}"
        )

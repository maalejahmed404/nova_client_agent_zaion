...
def numbers(text: str) -> set[str]:
    return {str(Decimal(n.replace(",", ".")).normalize()) for n in re.findall(r"\d+(?:[.,]\d+)?", text)}


def problems(table: dict, notice: str, generated: TableText) -> list[str]:
...
def _input(chunk: Chunk, table: dict) -> tuple[str, str]:
    lines = chunk.text.split("\n\n", 1)[0].splitlines()
    notice = next((l[len("Note : "):] for l in lines if l.startswith("Note : ")), "")
    records = "\n".join(" · ".join(f"{h} : {c}" for h, c in zip(table["headers"], r)) for r in table["rows"])
    return notice, f"{lines[0]}\n{lines[1]}\nNote : {notice or '(aucune)'}\n\nTableau :\n{records}"


def generate(chunk: Chunk, table: dict, offline: bool = False) -> str | None:
    notice, prompt = _input(chunk, table)
    key = hashlib.sha256(f"{get_settings().chat_model}|{PROMPT_VERSION}|{prompt}".encode()).hexdigest()
    ...
    if path.exists():
        ...
        chat = llm.get_chat(callbacks=[llm.UsageLogger()], tags=["table_text"])
        generated = llm.invoke_with_retry(chat.with_structured_output(TableText, method="function_calling"),
